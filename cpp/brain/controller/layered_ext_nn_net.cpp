...
void LayeredExtNNController::update(const std::vector<ActuatorPtr>& actuators,
				   const std::vector<SensorPtr>& sensors,
				   double t,
				   double step) 
{
...
boost::shared_ptr<LayeredExtNNConfig> LayeredExtNNController::getGenome()
{
...
void LayeredExtNNController::setGenome(boost::shared_ptr<LayeredExtNNConfig> Config)
{
	layers_ = Config->layers_;
	outputPositionMap_ = Config->outputPositionMap_;
	inputPositionMap_ = Config->inputPositionMap_;
