...
const double spline_controller::CYCLE_LENGTH = 5; // seconds
const unsigned int spline_controller::INTERPOLATION_CACHE_SIZE = 100; // number of data

spline_controller * spline_controller::GenerateRandomController(
    double noise_sigma,
...
spline_controller::spline_controller(unsigned int n_actuators,
                                     unsigned int n_spline_points,
                                     unsigned int interpolation_cache_size)
...
spline_controller::spline_controller(unsigned int n_actuators,
                                     unsigned int n_spline_points)
    : spline_controller(n_actuators, n_spline_points, INTERPOLATION_CACHE_SIZE)
{}


spline_controller::~spline_controller()
{}

void spline_controller::update(const std::vector<ActuatorPtr> &actuators,
...
void spline_controller::generateOutput(const double time, double* output_vector)
{
...
void spline_controller::update_cache()
{
    this->Interpolate_cubic(policy.get(), interpolation_cache.get());
}

void spline_controller::Interpolate_cubic(Policy *const source_y, Policy* destination_y)
{
