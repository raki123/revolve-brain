#include "input_dependent_oscillator_neuron.h"
#include <cmath>
#include <iostream>
...
std::map< std::string, double > InputDependentOscillatorNeuron::getNeuronParameters()
{
...
void InputDependentOscillatorNeuron::setNeuronParameters(std::map< std::string, double > params)
{
	if (!params.count("rv:period") || !params.count("rv:phase_offset") || !params.count("rv:amplitude")) {
		std::cerr << "An `" << "Oscillator" << 
		"` neuron requires `rv:period`, `rv:phase_offset` and `rv:amplitude` elements." << std::endl;
