...
void RLPower::loadPolicy(std::string const policy_path)
{
  ...
  {
    std::cout
            << "Number of (n_spline_points) is not equal to (n_actuators * n_steps)!"
            << std::endl;
    return;
  }
    ...
    if (algorithm_type_ == "B" || algorithm_type_ == "D")
    {
      ...
      for (size_t i = 0; i < n_actuators_; i++)
      {
        ...
        for (size_t j = 0; j < source_y_size_; j++)
        {
          ...
          spline_point +=
                  ((policy1->at(i)[j] - (*current_policy_)[i][j])) * (fitness1 / total_fitness);
          spline_point +=
                  ((policy2->at(i)[j] - (*current_policy_)[i][j])) * (fitness2 / total_fitness);
      ...
      for (size_t i = 0; i < n_actuators_; i++)
      {
        ...
        for (size_t j = 0; j < source_y_size_; j++)
        {
          ...
          for (auto const &it : ranked_policies_)
          {
            ...
            spline_point +=
                    ((policy->at(i)[j] - (*current_policy_)[i][j])) * (fitness / total_fitness);
          }
        ...
        Policy *destination_y)
{
  ...
  for (size_t j = 0; j < n_actuators_; j++)
  {
